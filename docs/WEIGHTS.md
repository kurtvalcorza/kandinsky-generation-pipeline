...
- Local layout: `weights/kandinsky-2-2-prior/` holds the 14 manifest entries (10,574,964,619 bytes): `README.md`, `model_index.json`, `prior/config.json`, `prior/diffusion_pytorch_model.safetensors` (4,104,940,968 bytes, `PriorTransformer`), `image_encoder/config.json`, `image_encoder/model.safetensors` (3,689,912,664 bytes, CLIP ViT-G/14), `image_processor/preprocessor_config.json`, `text_encoder/config.json`, `text_encoder/model.safetensors` (2,778,702,976 bytes, CLIP text encoder), `tokenizer/{tokenizer_config.json, vocab.json, merges.txt, special_tokens_map.json}`, and `scheduler/scheduler_config.json`.
