...
Rejected requests never import model libraries; the snapshots are verified and the dataset is validated
...
from kandinsky_generation_pipeline import validate_dataset, validate_prompts

PACKAGE = Path(__file__).resolve().parents[1] / "src" / "kandinsky_generation_pipeline"


def test_package_import_does_not_import_model_libraries(forbid_model_imports):
    import importlib

    import kandinsky_generation_pipeline

    importlib.reload(kandinsky_generation_pipeline)
